...
void process_bee(int sock, int n) {
    ...
    while (1) {
        c = BEE_OUT;
        if (sendto(sock, &c, 1, 0, (struct sockaddr *) &echoServAddr, sizeof(echoServAddr)) != 1) {
        ...
        totalBytesRcvd = 0;
        while (totalBytesRcvd < 1) {
            if ((bytesRcvd = recvfrom(sock, rcvBuff, 1, 0, (struct sockaddr *) &clientAddr, &clientAddrLen)) < 0) {
                exit_error("recvfrom() failed or connection closed prematurely\n");
            }
            totalBytesRcvd += bytesRcvd;
            if (rcvBuff[0] != BEE_IN) {
